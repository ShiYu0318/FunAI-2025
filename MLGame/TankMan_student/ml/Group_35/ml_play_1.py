...
MODEL_AIM_PATH = os.path.join(BASE_DIR, "aim_1.zip")
MODEL_CHASE_PATH = os.path.join(BASE_DIR, "chase_1.zip")
...
class MLPlay:
    ...
    def update(self, scene_info: dict, keyboard=[], *args, **kwargs):
        ...
        if dist < 295**2 and (
            ...
            if scene_info["y"] - abs(self.target_y) > 0:
                ...
                if scene_info["x"] - abs(self.target_x) > 0:
                    if scene_info["gun_angle"] != 0:
                        command = [AIM_RIGHT_CMD]
                    else:
                        command = [SHOOT]
                elif scene_info["x"] - abs(self.target_x) < 0:
    ...
    def get_obs_chase(
        self, player: str, target_x: int, target_y: int, scene_info: dict
    ) -> np.ndarray:
        ...
        obs = np.array(
            [float(tank_angle_index), float(angle_to_target_index)], dtype=np.float32
        )
    ...
    def get_obs_aim(
        ...
        obs = np.array(
            [float(gun_angle_index), float(angle_to_target_index)], dtype=np.float32
        )
        return obs
