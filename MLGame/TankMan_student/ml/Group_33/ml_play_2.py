...
# # tmp 只是檔名哈哈
# ########################################################################################
# tmp = ".\log\sb3\train_resupply_Feb06_2025_10-16-40\weights\rl_model_2000_steps" # 可以往前衝
# ########################################################################################
# tmp = "train_resupply_Feb06_2025_13-56-43"
# MODEL_AIM_PATH = os.path.join(MODEL_DIR, rf".\Group_33\model_2a.zip") # aim
# tmp = "train_resupply_Feb06_2025_10-16-40"
# MODEL_CHASE_PATH = os.path.join(MODEL_DIR, rf".\Group_33\model_2b.zip") # chase
...
class MLPlay:
    ...
    def update(self, scene_info: dict, keyboard=[], *args, **kwargs):
        ...
        for target in scene_info['competitor_info']:
            if self.distance(scene_info['x'], scene_info['y'], self.target_x, self.target_y) > self.distance(scene_info['x'], scene_info['y'], target['x'], target['y']):
        ...
        # Randomly switch between model_aim and model_chase
         # flag
        if self.distance(scene_info['x'], scene_info['y'], self.target_x, self.target_y) < 200:
            obs = self._get_obs_aim() # SHOOT
            action, _ = self.model_aim.predict(obs, deterministic=True)
            command = COMMAND_AIM[action]
