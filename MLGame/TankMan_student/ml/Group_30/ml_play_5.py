...
MODEL_AIM_PATH = os.path.join(MODEL_DIR, "model_5a.zip")
MODEL_CHASE_PATH = os.path.join(MODEL_DIR, "model_5b.zip")
...
class MLPlay:
    def __init__(self, ai_name, *args, **kwargs):
        ...
        self.target_x = None
        self.target_y = None
        self.searching_new_enemy = False
    ...
    def update(self, scene_info: dict, keyboard=[], *args, **kwargs):
        ...
        if scene_info["competitor_info"]:
            nearest_enemy = scene_info["competitor_info"][1]
        else:
            print("No valid target available.")
            return "RESET"

        if self.searching_new_enemy:
            # Turn 90 degrees before searching for a new enemy
            if scene_info["angle"] % 180 != 0:
            ...
            if nearest_enemy["y"] + 100 > 450:
                self.target_y = nearest_enemy["y"] - 50
            else:
                self.target_y = nearest_enemy["y"] + 50
