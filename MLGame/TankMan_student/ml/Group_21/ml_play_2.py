...
MODEL_AIM_PATH = os.path.join(BASE_DIR, "aim2.zip")
MODEL_CHASE_PATH = os.path.join(BASE_DIR, "chase2.zip")
...
class MLPlay:
    ...
    def update(self, scene_info: dict, keyboard=[], *args, **kwargs):
        ...
        if target["distance"] < 270 and (abs(player_x - target["x"]) < self.margin_of_error or abs(player_y - target["y"]) < self.margin_of_error):
